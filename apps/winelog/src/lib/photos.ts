...
import { isAllowedPhotoType, MAX_PHOTO_BYTES, MAX_PHOTOS_PER_WINE } from "./photo-limits";

export const PHOTO_LIMITS = { maxBytes: MAX_PHOTO_BYTES, maxCount: MAX_PHOTOS_PER_WINE } as const;
...
export async function storePhotos(files: readonly File[], wineId: number): Promise<StoredPhoto[]> {
	const targets = files.filter((f) => f && f.size > 0);
	if (targets.length === 0) return [];
	if (targets.length > MAX_PHOTOS_PER_WINE) {
		throw new Error(`写真は一度に ${MAX_PHOTOS_PER_WINE} 枚までです`);
	}
	...
	for (const file of targets) {
		const key = `wines/${wineId}/${crypto.randomUUID()}.${extensionFor(file.type)}`;
