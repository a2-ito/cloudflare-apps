...
/**
 * ログイン中のユーザ ID。未ログインなら null。
 * Route Handler ではリダイレクトではなく 401 を返したいので requireUser とは分ける。
 */
export async function currentUserId(): Promise<number | null> {
	...
	const db = await getDb();
	const row = await upsertUser(db, { email, name: session.user?.name ?? null, image: session.user?.image ?? null });
	return row.id;
}
