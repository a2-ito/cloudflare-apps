 ...
 */
export function PhotoInput({ name, existing = 0 }: { name: string; existing?: number }) {
	// すでに付いている写真と合わせて上限に収める
	const maxFiles = Math.max(0, MAX_PHOTOS_PER_ENTRY - existing);
	...
	const addFiles = useCallback(async (files: readonly File[], source: "select" | "paste") => {
		...
		try {
			...
			setPicked((current) => {
				...
				if (room <= 0) {
					setError(`写真は ${maxFiles} 枚までです`);
					return current;
				}
				if (shrunk.files.length > room) setError(`写真は ${maxFiles} 枚までなので、${room} 枚だけ追加しました`);
	...
	return (
		<div className="space-y-2">
			...
			<p className="text-xs text-zinc-500">
				最大 {maxFiles} 枚。画像をコピーして、この画面で貼り付けても追加できます（送信前に長辺 {MAX_EDGE}px へ縮小）
			</p>
			{busy && (
