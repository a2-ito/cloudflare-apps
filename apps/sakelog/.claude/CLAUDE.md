...
お酒の記録アプリ「さけログ」。Next.js 16 (App Router) を Cloudflare Workers で動かし、
...
- **価格は最小通貨単位の整数**（`price_minor`）。浮動小数の誤差を持ち込まない。
  価格が無いときは通貨も残さない
- **日付は 2 種類**。`drunk_at` は飲んだ日（変換しない・`null` は未開栓）、`created_at` /
  `updated_at` は監査用の UTC
- **評価の軸は `src/lib/ratings.ts` の 1 か所**。DB の列・フォーム・表示をここから
  組み立てる。軸を足すときはここと `src/db/schema.ts` を直す。未評価は `null`
- **種類の定義は `src/lib/categories.ts` の 1 か所**。保存する値・入力欄・一覧のタブ・
  ガイド画面をここから組み立てる。種類を増やすのはこの配列に 1 つ足すだけ
- **種類ごとの項目は、その種類で意味を持つものだけ保存する**（`allowsField`）。
  フォームに出していない値が送られても捨てる。種類を変えたときに前の値を残さないため
- **材料は別テーブル**（`drink_ingredients`）。品種・ホップ・酒米・ボタニカルを同じ形で持つ。
  入力はカンマ区切り 1 欄で受け、`src/lib/ingredients.ts` で分解する。書いた順が表示順
- **ガイド（`/guide`）は `categories.ts` の note から作る**。解説を別に持たない
- **外部から貼られた URL は検証してから出す**（`src/lib/shop-url.ts`）。そのまま
...
- **`src/lib/` は Cloudflare の env に触れるものと触れないものを分ける**。
  クライアントから読むモジュール（`photo-limits` / `categories` / `ratings` /
  `ingredients` / `shop-url` など）は
...
- DOM を動かすテストは持てない。コンポーネントは、壊れると気づきにくい約束だけを
  ソースを読む形のテストで守る（`drink-form.test.ts` / `photo-gallery.test.ts`）
